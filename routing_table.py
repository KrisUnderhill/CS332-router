...
class RoutingTable:
    ...
    def add_route(self, ifaces: list, netaddr: L3Addr, mask_numbits: int, nexthop: L3Addr):
        ...
        # Iterate over ifaces to find the iface the nexthop address is accessible through.
        # Store iface in out_iface.
        out_iface = None
        for iface in ifaces:
            #if iface.get_netaddr() == nexthop.network_part_as_L3Addr(iface.get_mask()): # TODO Same network call?
            if iface.on_same_network(nexthop): # TODO Same network call?
                out_iface = iface
        # raise ValueError if it is not accessible out any interface. 
        if out_iface == None:
            raise ValueError("nexthop address must be avaiable through some iface")
    ...
    def get_best_route(self, dest: L3Addr) -> RoutingTableEntry:  # or None
        ...
        for entry in self._entries:
            if dest.network_part_as_L3Addr(entry.mask_numbits) == entry.destaddr: # if matches
                if ret == None:  
                    ret = entry # set to entry regardless if is currently None
                else:
                    if ret.mask_numbits < entry.mask_numbits: 
                        ret = entry # update if matches current entry with longer prefix
        return ret
