...
class L3Addr:
    ...
    def __init__(self, val):
        ...
        if isinstance(val, str):
            ...
            res = 0
            for octet, mult in zip(parts, [24, 16, 8, 0]):
                res += int(octet) * (2 ** mult)
            if res > 2 ** 32 -1:
    ...
    def host_part_as_int(self, mask_numbits: int) -> int:
        mask = maskToHostMask(mask_numbits) # network mask bitwise not = host mask
        return self._as_int & mask
    ...
    def is_bcast(self) -> bool:
        return self.as_int() == 2**32-1 # Addr == "255.255.255.255"
