...
class Router:
    ...
    def add_interface(self, iface: L3Interface):
        ...
        # TODO This should not be get_addr -> I'm not trying to send to the address of the interface
        # TODO: Use 0.0.0.0 as addr -> assuming if we're adding an interface it has to be local?
    ...
    def route_packet(self, pkt: L3Packet, incoming_iface: L3Interface) -> int:
        ...
        # NOTE: printing out what the algorithm is doing just before each return statement.

        # Drop packet if checks are true
        # Check if dest is broadcast
        if pkt.dest.is_bcast(): 
            print(f'{pkt} dropped because dest is bcast')
            return None
        
        # Check if dest is on incoming network
        if incoming_iface.on_same_network(pkt.dest):
            print(f'{pkt} dropped because dest is on same network as incoming_iface')
            return None

        for iface in self._ifaces:
            # Check if dest is a directed broadcast
            if pkt.dest == iface.get_directed_bcast_addr():
                print(f'{pkt} dropped because dest is directed bcast')
                return None

            # Check if packet is addressed to one of the interfaces
            if iface.get_addr() == pkt.dest:
                print(f'{pkt} accepted to be proccessed on this device directed to iface {iface.get_number()}')
                return None

        # Decrement ttl before forwarding 
        pkt.ttl -= 1

        # Check that ttl != 0 before forwarding
        if pkt.ttl == 0:
            print(f'{pkt} dropped because ttl = 0')
            return None
